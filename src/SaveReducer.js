  ...
  function saveReducer(save, action) {
    switch (action.type) {
      ...
      case "NEXTTICK": {
        ...
        if (energytime === 0) {
          save.energy = Math.min(energy + 10, level * 100);
          save.energytime = 30000;
        } else {
          save.energytime = Math.max(0, energytime - action.tms);
        }
        ...
        if (yardtime === 0) {
          ...
          save.yardtime = Math.max(0, yardtime - action.tms);
        }
        ...
        Array.from(garage.values()).forEach((gv) => {
          ...
          if (stage > -1 && stage < parts.length) {
            if (parts[stage].time === 0) {
              ...
              parts[stage].time = Math.max(0, parts[stage].time - action.tms);
            }
          }
        });
        ...
        if (market !== null) {
          if (marktime === 0) {
            ...
            save.marktime = Math.max(0, marktime - action.tms);
          }
        }
